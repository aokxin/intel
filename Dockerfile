...
RUN echo 'chm868' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/chm868.sh

RUN echo 'cd ' >>/chm868.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/chm868.sh

RUN echo 'cd /noVNC-1.2.0' >>/chm868.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/chm868.sh

RUN chmod 755 /chm868.sh
